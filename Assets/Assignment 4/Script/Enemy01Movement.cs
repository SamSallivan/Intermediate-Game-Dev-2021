...
public class Enemy01Movement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!isStunned)
        {
            RaycastHit2D detection = Physics2D.Raycast(transform.position, new Vector2(side, 0f), 15f, playerLayer);
            if (detection.collider && !isStunned)
            {
                isAttacking = true;
    ...
    void OnDrawGizmos()
    {
        ...
        {
            Debug.DrawRay(transform.position, new Vector2(side, 0f) * 15f, Color.red);
        }
        else
        {
            Debug.DrawRay(transform.position, new Vector2(side, 0f) * 15f, Color.blue);
        }
    }
}
